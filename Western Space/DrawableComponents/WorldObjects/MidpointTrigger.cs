...
namespace WesternSpace.DrawableComponents.WorldObjects
{
    ...
    /// </summary>
    public class MidPointTrigger : WorldObject, ISpriteCollideable
    {
        private Texture2D texture;

        public MidPointTrigger(World world, SpriteBatch spriteBatch, Vector2 position)
        ...
        public void OnSpriteCollision(ISpriteCollideable objectCollidedWith)
        {
            if (objectCollidedWith == World.Player)
            {
                if (!playerColliding)
                {
                    ...
                    StoryboardScreen sbs = new StoryboardScreen("MidPoint", "Ending", @"StoryboardXML\MidPointStoryboard");
                    ScreenManager.Instance.ScreenList.Add(sbs);
                    ScreenTransition st = new ScreenTransition("Game", "MidPoint", 0.2f, 0.2f, false, false);
